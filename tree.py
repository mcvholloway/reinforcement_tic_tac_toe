...
class Node:
    def __init__(self, address, tree, game, current_state, parent = None):
        # Address is sequence of (state, action, state, action, ...)
        self.parent = parent
        self.game = game
        self.tree = tree
        ...
        for action in actions:
            # [total reward, number of times taken this action]
            di[action] = [0,0]
        self.actions = di
    ...
    def make_child(self, action):
        next_state = self.game.find_next_state(self.current_state, action)[0]
